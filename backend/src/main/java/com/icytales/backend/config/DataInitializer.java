...
public class DataInitializer {
    @Bean
    CommandLineRunner seedProducts(ProductRepository repository, UserRepository userRepository) {
        return args -> {
            User admin = userRepository.findByEmailIgnoreCase("admin@icytales.com").orElse(null);
            if (admin == null) {
                admin = new User();
                admin.setName("IcyTales Admin");
                admin.setEmail("admin@icytales.com");
                admin.setUserId("admin");
                admin.setPassword(new BCryptPasswordEncoder().encode("Admin@12345"));
                admin.setRole("ADMIN");
